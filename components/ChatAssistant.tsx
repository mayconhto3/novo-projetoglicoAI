...
export const ChatAssistant: React.FC<ChatAssistantProps> = ({ user, readings, isOpen, onClose, onDataUpdate, onMealEntry, onInsulinEntry }) => {
  ...
  const handleAiActionFeedback = (actionData: any) => {
    ...
    if (feedbackItems.length > 0) {
      ...
      // Award XP
      if (hasMeal && onMealEntry) {
        setTimeout(() => onMealEntry(), 500);
      }
      if (hasInsulin && onInsulinEntry) {
        setTimeout(() => onInsulinEntry(), 500);
      }
    }
  };
